...
设训练数据为 $\{ \mathbf{x}_i \}$，我们的目的是生成新的样本 $\{\mathbf{x}_i^* \}$，使得新样本和原始数据无法分辨。一个新样本 $\mathbf{x}_j^*$ 的生成过程大致为，首先从简单的基本分布（如标准正态分布）中选择一个隐变量（latent variable）$\mathbf{z}_j$ ，然后放到网络中得到 $\mathbf{x^*=g[z_j,\theta]}$ ，其中 $\mathbf{\theta}$ 是网络参数。这个网络即生成器（generator），网络学习的目标是找到最好的 $\mathbf{\theta}$ ，使得生成的样本看起来和真实原始数据非常相似。
...
$$
\begin{array}{l}
D_{JS}\left[Pr(\mathbf{x}^*)||Pr(\mathbf{x}) \right] & = & \frac{1}{2}D_{KL} \left[Pr(\mathbf{x}^*)||\frac{Pr(\mathbf{x}^*)+Pr(\mathbf{x})}{2} \right] + \frac{1}{2}D_{KL} \left[Pr(\mathbf{x})||\frac{Pr(\mathbf{x}^*)+Pr(\mathbf{x})}{2} \right] \\
